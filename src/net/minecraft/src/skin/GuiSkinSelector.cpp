...
void GuiSkinSelector::initGui()
{
    ...
    std::string defLabel = (currentPackIndex == 0) ? "> Default Skins <" : uiText("Default Skins");
    ...
    buttonPlayer2Skin = new GuiButton(BUTTON_ID_PLAYER2, p2BtnX, btnY, p2BtnWidth, p2BtnHeight, uiText("Choose 2nd Player Skin"));
...
void GuiSkinSelector::drawFrontPreview(const SkinEntry *skin, float x, float y, float w, float h, float alpha)
{
    ...
    if (!skin->isCustom && !skin->frontPath.empty() &&
        skin->frontPath != SkinManager::getDefaultSkinTexture())
    {
